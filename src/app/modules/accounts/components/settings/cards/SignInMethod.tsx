...
const SignInMethod: React.FC = () => {
  ...
  const [hasErrors2, setHasErrors2] = useState<boolean | undefined>(undefined)
  ...
  const formik2 = useFormik<IUpdatePassword>({
    ...
    onSubmit: (values) => {
      setLoading2(true)
      setHasErrors2(undefined)
      setMessage2(undefined)
      setTimeout(async () => {
        // if (!auth || !auth.token) {
        //   return;
        // }
        ...
        try {
          alert('Has to be implemented')
          const { data } = await changePassword('email', values.currentPassword, values.newPassword)
          setMessage2(data.message)
          setHasErrors2(false)
        } catch (error: unknown) {
          if (axios.isAxiosError(error)) {
            setMessage2(error?.response?.data?.message ?? 'An unexpected error occurred.')
            setHasErrors2(true)
          }
        }

        setTimeout(() => setHasErrors2(undefined), 2500)
        ...
        <div className='card-body border-top p-9'>
          {/* <div className='d-flex flex-wrap align-items-center'>
            <div id='kt_signin_email' className={' ' + (showEmailForm && 'd-none')}>
              ...
              <div className='fw-bold text-gray-600'>{currentUser?.email ? currentUser.email : 'support@keneframes.com'}</div>
            </div>
          ...
          {hasErrors2 === true && (
            <div className='mb-lg-15 alert alert-danger'>
              <div className='alert-text font-weight-bold'>{message2}</div>
            </div>
          )}
