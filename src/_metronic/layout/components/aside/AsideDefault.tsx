...
const AsideDefault = () => {
  ...
  const hideSidebar = () => {
    if (pathname === "/account-setup" || pathname === "/crafted/pages/profile/overview" || pathname === "/crafted/account/settings" || pathname === "/crafted/account/overview" || pathname === "/dashboard") {
    ...
    >
      ...
      <div className='aside-primary d-flex flex-column align-items-lg-center flex-row-auto'>
        ...
        >
          <Link to='/chat-bot/chat'>
            <img src={toAbsoluteUrl('/media/logos/custom-2.svg')} alt='logo' className='h-35px' title="LighthouseGPT" />
          </Link>
        </div>
        {/* end::Logo */}
        {/* begin::Nav */}
      ...
      {config.aside.secondaryDisplay && (
        <>
          ...
          >
            <KTIcon iconName={(isMinimized) ? 'arrow-right' : 'arrow-left'} className='fs-2 rotate-180' />
          </button>
          {/* end::Aside Toggle */}
        </>
      )}
